...
namespace WCF_QuanLyKhachSanForm
{
    public partial class FormVatTu : Form
    {
        ...
        ServiceChiTietVatTuClient ctvatu = new ServiceChiTietVatTuClient();
        ServiceVatTuClient vatu = new ServiceVatTuClient();
        ...
        private void Show_Vattu()
        {
            ...
            IList<VatTuDTO> listVT=vatu.getListVatTuAll();
        ...
        {
            cmbVattu.DataSource = vatu.getListVatTuAll();
        ...
        {
            cmbLoaiPhong.DataSource = lphong.getListLoaiPhongAll();
        ...
        private void btnThemCTVT_Click(object sender, EventArgs e)
        {
            if (kiemtrabotri())
            {
                ...
                if (ctvatu.insertChiTietVatTu(ctvtDTO) == 1)
                {
                    Show_ChitietVT();
                }
                else
        ...
        private void btnXoaCTVT_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (MessageBox.Show("Bạn có chắc thực hiện việc này không", "Thông Báo", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                {
                    ...
                    if (ctvatu.deleteChiTietVatTu(MaVT, MaLP) == 1)
                    {
                        Show_ChitietVT();
                    }
                    else
        ...
        private void btnSuaCTVT_Click(object sender, EventArgs e)
        {
            ...
            if (soluong())
            {
                ...
                ctvtDTO.Soluong = int.Parse(numSL.Value.ToString());
                ctvatu.updateChiTietVatTu(ctvtDTO);
                Show_ChitietVT();
            }
        } 
